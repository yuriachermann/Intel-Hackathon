...
from app.utils.dataloaders import (
    IMG_FORMATS,
    VID_FORMATS,
    LoadImages,
    LoadScreenshots,
...
def run(
    image=None,  # file/dir/URL/glob/screen/0(webcam)
    ...
    # else:
    dataset = LoadImages(
        image, img_size=imgsz, stride=stride, auto=pt, vid_stride=vid_stride
    )
    ...
    for image, im, im0s, vid_cap, s in dataset:
        with dt[0]:
        ...
        with dt[1]:
            visualize = (
                increment_path(save_dir / Path("data/images").stem, mkdir=True)
                if visualize
                else False
            )
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if webcam:  # batch_size >= 1
                ...
                p, im0, frame = "data/images", im0s.copy(), getattr(dataset, "frame", 0)
